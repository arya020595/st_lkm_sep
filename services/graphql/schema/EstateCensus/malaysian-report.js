...
const { jadual18Resolvers } = require("./jadual18Resolvers");
...
const generateMalaysianReport = async (self, params, context) => {
  ...
  const sumAllColumnCodesStringValue = context => {
    ...
    // console.log("3", { columnCodes });

    let estateIds = context.row.estateIds;
    // console.log(estateIds);
    // console.log(context.row);
    let jadual = context.row.jadual;
    let sum = 0;
  ...
  const REPORT_SPECS = {
      ...
      RESOLVE_ROWS: async () => {
        ...
        for (const year of YEARS) {
          ...
          if (!params.segment) {
            ...
            allEstateInformations = allEstateInformations.filter(
              est => est.stateCode === "12",
            );
          } else if (params.segment === "Sarawak") {
            allEstateInformations = allEstateInformations.filter(
              est => est.stateCode === "13",
            );
          } else if (params.segment === "Semenanjung") {
            allEstateInformations = allEstateInformations.filter(
              est => est.stateCode !== "13" && est.stateCode !== "12",
            );
          }
          for (const info of allEstateInformations) {
            ...
            let maklumatBorang = indexedEstateCensusMaklumatBorang.where({
              estateId: info.estateId,
              // censusYear: year + 1,
              censusYear: year,
            });
            maklumatBorang = maklumatBorang.filter(
              m =>
                m.estateStatus === "Aktif" ||
                m.estateStatus === "Terbiar" ||
                m.estateStatus === "Tiada",
            );
            ...
            const found = maklumatBorang.find(
              m => m.estateId === info.estateId,
            );
            if (found) {
              const foundValue = indexedValues.find({
              ...
              luasHektarDitanam += foundValue.value || 0;
          ...
          rows.push({
            ...
            indexedEstateCensusMaklumatBorang,
          });
        }
        ...
        [
          ...
          },
          {
            ...
            resolveValue: context => {
              ...
              let valueCocoaMonitor = 0;
              let value = 0;
              const stringSum = "" + sum;
              const splitted = stringSum.split(".");
              if (splitted.length === 1) {
                valueCocoaMonitor = Math.round(sum);
              } else {
                if (parseInt(splitted[1]) >= 60) {
                  valueCocoaMonitor = Math.round(sum);
                } else {
                  valueCocoaMonitor = parseInt(splitted[0]);
                }
              }
              value = context.row.luasHektarDitanam + valueCocoaMonitor;
          ...
          },
          {
            ...
            resolveValue: context => {
              ...
              for (const ct of ctx.row.estateInformations) {
                ...
                if (!tmpVal || !tmpVal.value) continue;
                // console.log({
                //   estateId: ct.estateId,
                //   censusYear: ctx["row"].year,
                //   value: tmpVal.value,
    ...
    "Jadual 6": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        const allEstateInformations = await context
          ...
          .toArray();
        // console.log("allEstateInformations", allEstateInformations.length);
        indexedEstateInformations = new FlexSearch({
          tokenize: "strict",
          doc: {
            id: "_id",
            field: ["estateId", "stateCode"],
          },
        });
        ...
        TARAF_SAH_LEGAL_STATUS = TARAF_SAH_LEGAL_STATUS.map(legalStatus => {
          ...
          if (!params.segment) {
            ...
            const tmpEstateIds = [...foundTarafSah.map(i => i.estateId)];
            let estateIds = [];
            for (const estId of tmpEstateIds) {
              const foundEstateInfo = indexedEstateInformations.find({
                estateId: estId,
                stateCode: "12",
              });
              if (foundEstateInfo) estateIds.push(estId);
            }
            return {
              legalStatus,
              estateIds,
            ...
            for (const estId of tmpEstateIds) {
              ...
              if (foundEstateInfo) {
                if (
                  foundEstateInfo.stateCode !== "12" &&
                  foundEstateInfo.stateCode !== "13"
                ) {
                  estateIds.push(estId);
                }
              }
            }
            return {
      ...
      COLUMNS: [
        ...
        [
          ...
          },
          {
            ...
            columnCodes: ["A01422"],
            statsPrecision: 2,
            statsPrecisionOnJumlah: 0,
            resolveValue: context => {
              const val = sumAllColumnCodesValue(context);
              return {
                text: lodash.round(val.value, 0), //context.row.stateName,
                value: val.value, //context.row.stateName,
              };
            },
            // resolveValue: sumAllColumnCodesValue,
            // resolveValue: context => {
      ...
      RESOLVE_ROWS: async () => {
        ...
        if (params.segment === "Sabah" || params.segment === "Sarawak") {
          ...
          for (const state of allEstateCensusStateDistricts) {
            allEstateInformations = indexedEstateInformations.where({
          ...
          return allDistricts.map(i => {
            const estateIds = (i.estateInformations || []).map(
              is => is.estateId,
            );
        ...
        let allStates = [];
        if (params.segment === "Semenanjung") {
          allEstateCensusStateCodes = allEstateCensusStateCodes.filter(
            st => st.stateCode !== "12" && st.stateCode !== "13",
          );
        }
        ...
        [
          ...
          },
          {
            ...
            resolveValue: context => {
              const val = sumAllColumnCodesValue(context);
              return {
                text: lodash.round(val.value,2), //lodash.round(val.value, 0), //context.row.stateName,
                value: val.value, //context.row.stateName,
              };
            },
            // statsPrecision: 0,
    ...
    "Jadual 7": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        let estateIds = [];
        if (!params.segment) {
          estateIds = [...new Set(foundValues.map(v => v.estateId))];
        } else {
          const tmpEstateIds = [...new Set(foundValues.map(v => v.estateId))];
        ...
        SIZES = SIZES.filter(size => size.estateIds.length > 0);
        // throw {};
    ...
    "Jadual 13": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        if (!params.segment) {
          ...
          for (const estId of tmpEstateIds) {
            let foundEstateInfo = null;
            if (params.segment !== "Semenanjung") {
              foundEstateInfo = indexedEstateInformations.find({
                estateId: estId,
                stateCode,
              });
              if (foundEstateInfo) estateIds.push(estId);
          ...
          },
          {
            ...
            resolveValue: context => {
              ...
              return {
                ...
                valueD03401,
                value,
                text: lodash.round(value, 0), //value,
              };
            },
            resolveTotalValue: context => {
              let sumD03201 = 0,
      ...
      RESOLVE_ROWS: async () => {
        ...
        for (const year of YEARS) {
          ...
          if (!params.segment) {
            ...
            for (const estId of tmpEstateIds) {
              const found = values.find(v => v.estateId === estId);
              if (found) {
                estateIds.push(estId);
              }
            }
            ...
            estateIds = lodash.uniq(values.map(v => v.estateId));
    ...
    "Jadual 3": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        if (params.segment === "Sabah" || params.segment === "Sarawak") {
          ...
          // console.log("allDistricts", allDistricts.length)
          ...
          //
          {
            ...
            resolveValue: context => {
              ...
              if (!JenisHakMilik) {
                ...
                return {
                  value,
                  text: lodash.round(value, 0), //formatNumber(value, context.spec.statsPrecision || 0),
                };
              } else {
                return {
                  value: 0,
    ...
    "Jadual 4": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        if (params.segment && params.segment === "Semenanjung") {
          allEstateInformations = allEstateInformations.filter(
            st =>
              String(parseInt(st.stateCode)) !== "12" &&
              String(parseInt(st.stateCode)) !== "13",
          );
        }
        for (const info of allEstateInformations.filter(est =>
          params.segment && params.segment === "Sabah"
            ? String(parseInt(est.stateCode)) === "12"
            : params.segment === "Sarawak"
            ? String(parseInt(est.stateCode)) === "13"
            : est.stateCode,
        )) {
          // Aktif Saja
        ...
        let tarafSahLegalStatus = {
          ...
          "": { label: "Lain-lain" },
          "Hakmilik Perseorangan": { label: "Lain-lain" },
          "Syarikat Kerjasama": { label: "Lain-lain" },
          "Hak Perseorangan": { label: "Lain-lain" },
          "Pertubuhan Tanpa Untung": { label: "Lain-lain" },
        };
          ...
          for (const group of [
            ...
            for (const estInfo of foundEstateInformations) {
              ...
              for (const code of ["Q00310", "Q00318", "Q00337"]) {
                ...
                if (code === "Q00310") {
                  valueQ00310 += foundValue.value || 0;
                } else if (code === "Q00318") {
                  valueQ00318 += foundValue.value || 0;
                } else if (code === "Q00337") {
                  valueQ00337 += foundValue.value || 0;
                }
              }
        ...
        let rowsThatHasValues = [];
        for (const key of Object.keys(groupObject)) {
          const obj = groupObject[key];
          if (obj.totalJumlah > 0) {
            rowsThatHasValues.push(obj);
          }
        }
        ...
        returnColums = returnColums.filter(ret => {
          const found = rowsThatHasValues.find(row => row.label === ret.label);
          if (found) {
            return found;
          }
        });
    ...
    "Jadual 5": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        if (params.segment === "Sabah" || params.segment === "Sarawak") {
          ...
          allDistricts = allDistricts.map(state => {
            let estateIds = (state.estateInformations || []).map(
              i => i.estateId,
            );
            estateIds = estateIds.filter(estateId => {
              const foundEstateCensusMaklumatBorang =
        ...
        if (params.segment === "Sabah" || params.segment === "Sarawak") {
          ...
          for (const state of allEstateCensusStateDistricts) {
            ...
            for (const info of allEstateInformations) {
              ...
              const foundValue = indexedValues.find({
                // estateInformationId: info.estateId,
                estateId: info.estateId,
                // code: columnCode,
              });
              // console.log({ info }, !!foundValue);
      ...
      COLUMNS: [
        ...
        [
          ...
          },
          {
            ...
            statsPrecision: 2,
            resolveValue: context => {
              let result = sumAllColumnCodesValue(context);
              return {
                text: formatNumber(result.value, 2, "."),
                value: result.value,
                fontSize: 7.5,
              };
            },
          ...
          },
          {
            ...
            columnWidth: 43,
            hideColumnCodes: true,
            columnCodes: ["M10016", "M10017", "M10038", "M10039"],
            statsPrecision: 2,
            resolveValue: context => {
              let result = sumAllColumnCodesValue(context);
              return {
      ...
      COLUMNS: [
        ...
        [
          ...
          },
          {
            ...
            resolveValue: context => {
              return {
                ...
                text: formatNumber(
                  lodash.round(context.row.mudaNilai, 2),
                  2,
                  ".",
                ),
              };
          ...
          // --------------------------
          {
            ...
            resolveValue: context => {
              return {
                value: context.row.cukupUmurHektar,
                text: formatNumber(
          ...
          },
          {
            ...
            resolveValue: context => {
              return {
                value: context.row.cukupUmurLiter,
    ...
    "Jadual 14": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        let estateIds = allEstateCensusMaklumatBorang.map(i => i.estateId);

        if (params.segment === "Sabah") {
          let tmpEstateIds = [];
          for (const estId of estateIds) {
            const foundInfo = indexedEstateInformations.find({
              estateId: estId,
            });
            if (foundInfo.stateCode === "12") tmpEstateIds.push(estId);
          }
          estateIds = tmpEstateIds;
        } else if (params.segment === "Sarawak") {
          let tmpEstateIds = [];
          for (const estId of estateIds) {
            const foundInfo = indexedEstateInformations.find({
            ...
            if (foundInfo.stateCode === "13") tmpEstateIds.push(estId);
          }
          estateIds = tmpEstateIds;
        } else if (params.segment === "Semenanjung") {
          let tmpEstateIds = [];
          for (const estId of estateIds) {
            ...
            if (foundInfo.stateCode !== "13" && foundInfo.stateCode !== "12")
              tmpEstateIds.push(estId);
          }
          estateIds = tmpEstateIds;
        }
        ...
        return [
          ...
          },
          {
            ...
            columnCodes: {
              ...
              "Warga Negara Malaysia - Jumlah":
                // "M09310+M09311a+M09312a+M09311b+M09312b+M09332+M09333a+M09334a+M09333b+M09334b+M09710+M09711a+M09712a+M09711b+M09712b+M09732+M09733a+M09734a+M09733b+M09734b",
                "M09310+M09311a+M09312a+M09311b+M09312b+M09332+M09333a+M09334a+M09333b+M09334b",
      ...
      COLUMNS: [
        ...
        [
          ...
          },
          {
            ...
            statsPrecision: 2,
            resolveValue: context => {
              const result = sumAllColumnCodesStringValue(context);
              if (result.value) {
                return {
                  text: formatNumber(result.value, 2, "."),
                  value: result.value,
    ...
    "Jadual 18": {
      ...
      RESOLVE_ROWS: async () => {
        ...
        if (!params.segment) {
          ...
          let rows = jadual18Resolvers(tmpRows, indexedValues, estateIds);
      ...
      COLUMNS: [
        ...
        [
          ...
          // ------------------------------
          {
            ...
            resolveValue: context => {
              if (!params.segment) {
                return sumAllColumnCodesStringValue(context);
              }
              return {
                text: formatNumber(
                  context.row.columnCodes[context.spec.key],
                  2,
                  ".",
                ),
                value: context.row.columnCodes[context.spec.key],
              };
            },
            // valueAlignment: "center",
          },
    ...
    "Jadual 19": {
      ...
      RESOLVE_ROWS: async () => {
          ...
          },
          {
            ...
            resolveValue: {
              Nilai: context => {
                ...
                let value = 0;
                if (!valueA00311 || valueA00311.value === 0) {
                  value = 0;
                } else {
                  value =
        ...
        if (!params.segment) {
          ...
          allEstateInformations = lodash.uniq(
            allEstateInformations.map(est => est.estateId),
          );

          let filteredEstateInformations = [];
          for (const estateId of allEstateInformations) {
            let maklumatBorang = indexedEstateCensusMaklumatBorang.where({
              estateId: estateId,
            });

            if (maklumatBorang.length === 0) {
              continue;
            } else {
              filteredEstateInformations.push(estateId);
            }
          }

          let rows = [];
          let counter = 0;
          for (let row of defaultRow) {
            counter += 1;

            if (counter === 1) {
              let value = 0;
              for (const estId of filteredEstateInformations) {
                const found = indexedValues.where({
                  estateId: estId,
                  code: "A01410",
                });
              ...
              for (const estId of filteredEstateInformations) {
                let valueJ06401 = 0;
                const foundJ06401 = indexedValues.where({
                  estateId: estId,
                  code: "J06401",
                });
                ...
                let valueJ06402 = 0;
                const foundJ06402 = indexedValues.where({
                  estateId: estId,
                  code: "J06402",
                });
                ...
                let valueJ06403 = 0;
                const foundJ06403 = indexedValues.where({
                  estateId: estId,
                  code: "J06403",
                });
              ...
              let value = 0;
              for (const estId of filteredEstateInformations) {
                let valueJ06402 = indexedValues.where({
                  estateId: estId,
                  code: "J06402",
                });

                let valueA00311 = indexedValues.where({
                  estateId: estId,
                  code: "A00311",
                });

                valueJ06402 = valueJ06402
                  .map(v => v.value || 0)
                  .reduce((acc, curr) => acc + curr, 0);
                valueA00311 = valueA00311
                  .map(v => v.value || 0)
                  .reduce((acc, curr) => acc + curr, 0);

                if (!valueJ06402 || !valueA00311) {
                  value = 0;
                } else {
                  value = value + valueJ06402 / valueA00311;
                }
              ...
              let value = 0;
              for (const estId of filteredEstateInformations) {
                let valueJ06403 = indexedValues.where({
                  estateId: estId,
                  code: "J06403",
                });
                ...
                if (!valueJ06403 || !valueA00311) {
                  value = 0;
                } else {
                  value = value + valueJ06403 / valueA00311;
                }
              ...
              for (const estId of filteredEstateInformations) {
                let valueJ06501 = 0;
                const foundJ06501 = indexedValues.where({
                  estateId: estId,
                  code: "J06501",
                });
                ...
                let valueJ06502 = 0;
                const foundJ06502 = indexedValues.where({
                  estateId: estId,
                  code: "J06502",
                });
                ...
                let valueJ06503 = 0;
                const foundJ06503 = indexedValues.where({
                  estateId: estId,
                  code: "J06503",
                });
              ...
              let value = 0;
              for (const estId of filteredEstateInformations) {
                let valueJ06502 = indexedValues.where({
                  estateId: estId,
                  code: "J06502",
                });

                let valueA00411 = indexedValues.where({
                  estateId: estId,
                  code: "A00411",
                });

                valueJ06502 = valueJ06502
                  .map(v => v.value || 0)
                  .reduce((acc, curr) => acc + curr, 0);
                valueA00411 = valueA00411
                  .map(v => v.value || 0)
                  .reduce((acc, curr) => acc + curr, 0);

                if (!valueJ06502 || !valueA00411) {
                  value = 0;
                } else {
                  value = value + valueJ06502 / valueA00411;
                }

                // if (!valueA00411 || valueA00411.value === 0) {
                //   value = 0;
                // } else {
                //   value =
                //     !valueJ06502?.value || !valueA00411?.value
              ...
              let value = 0;
              for (const estId of filteredEstateInformations) {
                let valueJ06503 = indexedValues.where({
                  estateId: estId,
                  code: "J06503",
                });
                ...
                if (!valueJ06503 || !valueA00411) {
                  value = 0;
                } else {
                  value = value + valueJ06503 / valueA00411;
                }
              ...
              for (const estId of filteredEstateInformations) {
                let valueJ06603 = 0;
                const foundJ06603 = indexedValues.where({
                  estateId: estId,
                  code: "J06603",
                });
                ...
                let valueJ06604 = 0;
                const foundJ06604 = indexedValues.where({
                  estateId: estId,
                  code: "J06604",
                });
              ...
              let value = 0;
              for (const estId of filteredEstateInformations) {
                let valueJ06604 = indexedValues.where({
                  estateId: estId,
                  code: "J06604",
                });

                let valueB01505 = indexedValues.where({
                  estateId: estId,
                  code: "B01505",
                });

                valueJ06604 = valueJ06604
                  .map(v => v.value || 0)
                  .reduce((acc, curr) => acc + curr, 0);
                valueB01505 = valueB01505
                  .map(v => v.value || 0)
                  .reduce((acc, curr) => acc + curr, 0);

                if (!valueJ06604 || !valueB01505) {
                  value = 0;
                } else {
                  value = value + valueJ06604 / valueB01505;
                }
              ...
              let value = 0;
              for (const estId of filteredEstateInformations) {
                let valueJ06603 = indexedValues.where({
                  estateId: estId,
                  code: "J06603",
                });
                ...
                if (!valueJ06603 || !valueB01505) {
                  value = 0;
                } else {
                  value = value + valueJ06603 / valueB01505;
                }
              ...
              for (const estId of filteredEstateInformations) {
                ...
                value = value + (valueL07807 ? valueL07807 / 1000 : 0);
              }
              ...
              for (const estId of filteredEstateInformations) {
                ...
                if (valueL07801 + valueL07802 > 0) {
                  value = value + (valueL07801 + valueL07802) / 1000;
                }
              }
              ...
              for (const estId of filteredEstateInformations) {
                ...
                value = value + (valueL07803 ? valueL07803 / 1000 : 0);
              }
              ...
              for (const estId of filteredEstateInformations) {
                ...
                value = value + (valueL07804 ? valueL07804 / 1000 : 0);
              }
              ...
              for (const estId of filteredEstateInformations) {
                ...
                value = value + (valueL07805 ? valueL07805 / 1000 : 0);
              }
              ...
              for (const estId of filteredEstateInformations) {
                ...
                value = value + (valueL07806 ? valueL07806 / 1000 : 0);
              }
              ...
              if (GROUPED_FOOTERS_CONFIG.includes("allStats")) {
                table.push([
                  ...COLUMN_SPECS.map((spec, specIndex) => {
                    ...
                    const lenVal = formatNumber(
                      lodash.round(
                        (allStats[specIndex] && allStats[specIndex].total) || 0,
                        spec.statsPrecision || 0,
                      ),
                      spec.statsPrecision || 0,
                      ".",
                    ...
                    if (
                      spec.statsPrecisionOnJumlah ||
                      spec.statsPrecisionOnJumlah === 0
                    ) {
                      return {
                        text: formatNumber(
                          lodash.round(
                            (allStats[specIndex] &&
                              allStats[specIndex].total) ||
                              0,
                            0,
                          ),
                          0,
