...
const resolvers = {
  Query: {
    allUnstructuredDocumentOthers: async (self, params, context) => {
      return await context
        .collection("UnstructuredDocumentOthers")
        .find({
  ...
  Mutation: {
    createUnstructuredDocumentOthers: async (self, params, context) => {
      ...
      const payload = {
        _id: uuidv4(),
        affectedCollectionName: "UnstructuredDocumentOthers",
      ...
      await context.collection("UnstructuredDocumentOthers").insertOne(newData);

      return "success";
    },
    updateUnstructuredDocumentOthers: async (self, params, context) => {
      assertValidSession(context.activeSession);
      if (!context.activeSession.User) {
        throw new Error("Session End. Please re-login");
      }
      ...
      await context.collection("UnstructuredDocumentOthers").updateOne(
        {
          _id: params._id,
        },
        {
    ...
    deleteUnstructuredDocumentOthers: async (self, params, context) => {
      assertValidSession(context.activeSession);
      if (!context.activeSession.User) {
        throw new Error("Session End. Please re-login");
      }
