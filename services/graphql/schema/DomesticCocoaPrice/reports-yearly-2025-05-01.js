...
const generateYearlyCocoaPriceReportForDomesticCocoaPrices = async (
  ...
  let groupedDomesticCocoaPrices = allDomesticCocoaPrices.reduce(
    (acc, curr) => {
      if (!acc[curr.date + curr.centreId]) {
        acc[curr.date + curr.centreId] = [];
      }
      acc[curr.date + curr.centreId].push(curr);
      return acc;
    },
    {},
  );
  ...
  allCentres = allCentres.filter(centre => {
    const diffYear = params.toYearIds[0] - params.fromYearIds[0];

    let totalPricesLength = 0;
    for (let i = 0; i <= diffYear; i++) {
      let prices = indexedDomesticCocoaPrices.where({
        centreId: centre._id,
        year: parseInt(params.fromYearIds[0]) + i,
      });

      prices = prices.filter(p => p[GRADE_CODES[params.gradeIds[0]]] > 0);
      totalPricesLength += prices.length;
    }

    return totalPricesLength > 0;
                ...
                allCentres.forEach(centre => {
                  ...
                  for (const price of prices) {
                    ...
                    if (value > 0) {
                      ...
                      let foundDuplicate =
                        groupedDomesticCocoaPrices[
                          price.date + price.centreId
                        ] || [];
                  ...
                  const decAvg =
                    totalPerCentre[centre._id].dec.length > 0
                      ...
                      : 0;
                  averageArray.push(decAvg);
                  if (averageArray.filter(a => a > 0).length === 0) {
                    throw new Error(`Empty data on ${currentYear}`);
                  }
                  stats[statCode].average =
                    (janAvg +
                      ...
                      novAvg +
                      decAvg) /
                    averageArray.filter(a => a > 0).length;
