...
const start = async () => {
  ...
  const allQuestionnaires = await collection(
  ...
  for (let item of allQuestionnaires) {
    ...
    // ################################
    let foundQuestions = await collection("SmallholderCensusQuestionnaireData")
      .find({
        _id: {
          $in: item.questionIds || [],
        },
    ...
    await collection("SmallholderCensusQuestionnaireData").updateOne(
      {
        _id: item._id,
      },
      {
