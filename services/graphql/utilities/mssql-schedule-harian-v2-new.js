...
const scheduleHarian = async ({ collection, transaction, mssqlPool }) => {
  ...
  for (const centre of centres) {
    ...
    const checkduplicate = await mssqlPool
      .request()
      .query(
        `SELECT * FROM dbo.hharian WHERE TARIKH = '${newData.date}' AND PUSAT = '${centre.code}'`,
      );
    ...
    const runQuery = `SELECT * FROM dbo.hharian WHERE PUSAT = '${centre.code}' AND TARIKH = '${newData.date}'`;
    ...
    if (total > 0) {
      ...
      try {
        const request = new sql.Request(transaction);
        const runQuery = `INSERT INTO dbo.hharian (
        recid, 
        TARIKH, 
        PUSAT, 
        BSH_TGG, 
    ...
    console.log("Saving dbo.hharian ");
