...
const GET_ESTATE_SELLER = gql`
  query allEstateCensusSeller($estateInformationId: String) {
    allEstateCensusSeller(estateInformationId: $estateInformationId) {
      _id
      estateId
      name
      address
...
const GET_RECORD_ID = gql`
  query latestRecordIdQuery {
    estateCensusSellerLatestRecordId
  }
`;

const CREATE_ESTATE_SELLER = gql`
  mutation createEstateCensusSeller(
    $estateId: String
    $name: String
    $address: String
    $zipCode: String
...
const UPDATE_ESTATE_SELLER = gql`
  mutation updateEstateCensusSeller(
    $_id: String!
    $estateId: String
    $name: String
    $address: String
...
`;
const DELETE_ESTATE_SELLER = gql`
  mutation deleteEstateCensusSeller($_id: String!) {
    deleteEstateCensusSeller(_id: $_id)
  }
`;
...
const EstateCensusSeller = () => {
  ...
  const [sellerList, setEstateCensusSellerList] = useState([]);
  ...
  const [createEstateCensusSeller] = useMutation(CREATE_ESTATE_SELLER);
  const [updateEstateCensusSeller] = useMutation(UPDATE_ESTATE_SELLER);
  const [deleteEstateCensusSeller] = useMutation(DELETE_ESTATE_SELLER);

  const recordId = data?.estateCensusSellerLatestRecordId || "";
  ...
  const fetching = async estateInfoId => {
    ...
    try {
      ...
      setEstateCensusSellerList(result.data.allEstateCensusSeller);
    } catch (err) {
      notification.handleError(err);
    }
    hideLoadingSpinner();
  ...
  return (
    <AdminArea urlQuery={router.query}>
      ...
      <FormModal
        title={`${!formData._id ? "New" : "Edit"} Seller`}
        ...
        onSubmit={async e => {
          ...
          try {
            ...
            if (!_id) {
              await createEstateCensusSeller({
                variables: {
                  ...formData,
                  recordId,
              ...
              await updateEstateCensusSeller({
                variables: {
                  ...formData,
                },
              });
      ...
      <div className="mt-28 mx-10">
        ...
        <div className="mt-4">
          <Table
            loading={false}
            columns={columns}
            data={sellerList.filter(
              b => b.censusYear === parseInt(router.query.estateYear),
            )}
            withoutHeader={true}
            onAdd={
              currentUserDontHavePrivilege(["Seller:Create"])
                ? null
                : allEstateInformation.length === 0
                  ? null
                  : e => {
            ...
            onRemove={
              currentUserDontHavePrivilege(["Seller:Delete"])
                ? null
                : async ({ rows }) => {
                  showLoadingSpinner();
                  try {
                    ...
                    if (yes) {
                      for (const row of rows) {
                        await deleteEstateCensusSeller({
                          variables: {
                            _id: row._id,
                          },
                        });
            ...
            customUtilities={
              currentUserDontHavePrivilege(["Seller:Update"])
                ? null
                : customUtilities
            }
...
export default withApollo({ ssr: true })(EstateCensusSeller);
