...
const REPORT = gql`
  mutation generateMalaysianReport(
    $title: String!
    $code: String!
    $year: Int!
  ) {
    generateMalaysianReport(title: $title, code: $code, year: $year)
  }
`;
...
const EstateCensus = () => {
  ...
  const [generateMalaysianReport] = useMutation(REPORT);
  // ------------------------------------------------------------------------
            ...
            onSubmit={async e => {
              ...
              try {
                ...
                if (typeof reportMutation === "string") {
                  let result = await generateMalaysianReport({
                    variables: {
                      ...filters,
                      code: reportMutation,
                      title: filters.type,
                  ...
                  const reportUrl =
                    result.data.generateMalaysianReport +
                    "?t=" +
