...
const REPORT = gql`
  mutation generateSarawakReport($title: String!, $code: String!, $year: Int!) {
    generateSarawakReport(title: $title, code: $code, year: $year)
  }
`;
...
const EstateCensus = () => {
  ...
  const [generateSarawakReport] = useMutation(REPORT);
  // ------------------------------------------------------------------------
  const reportMutations = {
    ...
    "Penggunaan Input (Baja)": "Jadual 17",
    "Penggunaan Input (Racun)": "Jadual 17",
  ...
  return (
    <AdminArea urlQuery={router.query} title="Sarawak Report">
      <Head>
        <title>Sarawak Report</title>
      </Head>
            ...
            onSubmit={async e => {
              ...
              try {
                ...
                if (typeof reportMutation === "string") {
                  let result = await generateSarawakReport({
                    variables: {
                      ...filters,
                      code: reportMutation,
                      title: filters.type,
                  ...
                  const reportUrl =
                    result.data.generateSarawakReport +
                    "?t=" +
