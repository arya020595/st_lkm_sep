...
const REPORT = gql`
  mutation generateSemenanjungReport(
    $title: String!
    $code: String!
    $year: Int!
  ) {
    generateSemenanjungReport(title: $title, code: $code, year: $year)
  }
`;
...
const EstateCensus = () => {
  ...
  const [generateSemenanjungReport] = useMutation(REPORT);
  // ------------------------------------------------------------------------
  ...
  return (
    <AdminArea urlQuery={router.query} title="Semenanjung Report">
      <Head>
        <title>Semenanjung Report</title>
      </Head>
            ...
            onSubmit={async e => {
              ...
              try {
                ...
                if (typeof reportMutation === "string") {
                  let result = await generateSemenanjungReport({
                    variables: {
                      ...filters,
                      code: reportMutation,
                      title: filters.type,
                  ...
                  const reportUrl =
                    result.data.generateSemenanjungReport +
                    "?t=" +
