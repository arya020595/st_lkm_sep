...
const REPORT = gql`
  mutation generateSarawakSmallholderReport(
    $title: String!
    $code: String!
    $year: Int!
  ) {
    generateSarawakSmallholderReport(title: $title, code: $code, year: $year)
  }
`;
...
const EstateCensus = () => {
  ...
  const [generateSarawakSmallholderReport] = useMutation(REPORT);
  // ------------------------------------------------------------------------
            ...
            onSubmit={async e => {
              ...
              try {
                ...
                if (typeof reportMutation === "string") {
                  let result = await generateSarawakSmallholderReport({
                    variables: {
                      ...filters,
                      code: reportMutation,
                      title: filters.type,
                  ...
                  const reportUrl =
                    result.data.generateSarawakSmallholderReport +
                    "?t=" +
