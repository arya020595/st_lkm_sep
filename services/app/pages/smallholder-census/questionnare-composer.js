...
const UPDATE = gql`
  mutation updateSmallholderCensusForm($_id: String!, $specs: JSON) {
    updateSmallholderCensusForm(_id: $_id, specs: $specs)
  }
`;

const GENERATE = gql`
  mutation generateSmallholderCensusFormPDF($formId: String!) {
    generateSmallholderCensusFormPDF(_id: $formId)
  }
`;
...
const Questionnare = () => {
  ...
  const [generateSmallholderCensusFormPDF] = useMutation(GENERATE);
  const [updateSmallholderCensusForm] = useMutation(UPDATE);
  ...
  useEffect(() => {
    if (!smallholderCensusForm?._id) return;
    setFormData({
  ...
  const handleSave = useCallback(async (e, newFormData, options = {}) => {
    ...
    try {
      await updateSmallholderCensusForm({
        ...
        <div>
          <Button
            ...
            onClick={async e => {
              ...
              showLoadingSpinner();
              try {
                let result = await generateSmallholderCensusFormPDF({
                  variables: {
                    formId: router.query.formId,
                  },
                });
                const pdfUrl =
                  result.data.generateSmallholderCensusFormPDF +
                  "?t=" +
