...
const REPORT = gql`
  mutation generateSabahSmallholderReport(
    $title: String!
    $code: String!
    $year: Int!
  ) {
    generateSabahSmallholderReport(title: $title, code: $code, year: $year)
  }
`;
...
const EstateCensus = () => {
  ...
  const [generateSabahSmallholderReport] = useMutation(REPORT);
  // ------------------------------------------------------------------------
            ...
            onSubmit={async e => {
              ...
              try {
                ...
                if (typeof reportMutation === "string") {
                  let result = await generateSabahSmallholderReport({
                    variables: {
                      ...filters,
                      code: reportMutation,
                      title: filters.type,
                  ...
                  const reportUrl =
                    result.data.generateSabahSmallholderReport +
                    "?t=" +
