...
const REPORT = gql`
  mutation generateGlobalTradeDataCocoaProductByRegionPercentageReport(
    $type: String
    $product: String
    $fromYear: String
    $toYear: String
...
const Report = () => {
  ...
  const [generateGlobalTradeDataCocoaProductByRegionPercentageReport] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateGlobalTradeDataCocoaProductByRegionPercentageReport(
                {
                  variables: {
                    ...filters,
                  },
