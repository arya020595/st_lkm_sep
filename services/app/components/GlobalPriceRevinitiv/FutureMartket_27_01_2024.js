...
const FUTURE_MARKET_BY_DATE = gql`
  query futureMarketByDate($date: String!) {
    futureMarketByDate(date: $date) {
      _id
      date
      Source {
        _id
...
const CREATE_FUTURE_MARKET = gql`
  mutation createFutureMarket(
    ...
    createFutureMarket(
      ...
      iccoPoundsterling: $iccoPoundsterling
      iccoUSD: $iccoUSD
      iccoEx: $iccoEx
      iccoPrice: $iccoPrice
...
const DELETE_FUTURE_MARKET = gql`
  mutation deleteFutureMarket($_id: String!) {
    deleteFutureMarket(_id: $_id)
  }
`;
...
const FutureMarket = ({ currentUserDontHavePrivilege }) => {
  ...
  useEffect(async () => {
    ...
    if (result.data.futureMarketByDate) {
      setFutureMarket({
        ...result.data.futureMarketByDate,
        sourceId: result.data.futureMarketByDate?.Source?._id || "",
        infoStatusId: result.data.futureMarketByDate?.InfoStatus?._id || "",
      });
    } else {
      setFutureMarket({
        different: 0,
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      setFutureMarket({
        ...futureMarketByDate,
        sgPrice: sgData.avg * sgData.exc,
        iccoPrice: futureMarketByDate.iccoUSD * futureMarketByDate.iccoEx,
      });
    }, 1000);
  ...
  if (data?.allFutureMarkets) {
    allFutureMarkets = data.allFutureMarkets;
  }
      ...
      <div className="grid grid-cols-4 mb-4">
        ...
        <div>
          ...
          <p className="font-bold">
            {data?.getLastUpdateFutureMarketReutersPrice || "-"}
          </p>
        </div>
        <div>
          <p>
          ...
          <p className="font-bold">
            {data?.getLastUpdateFutureMarketReutersPricePushedToWBC || "-"}
          </p>
        </div>
      </div>
