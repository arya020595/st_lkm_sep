...
const REPORT = gql`
  mutation generateDailyReportForGlobalICCOPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateDailyReportForGlobalICCOPrices] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateDailyReportForGlobalICCOPrices({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateDailyReportForGlobalICCOPrices +
              "?t=" +
