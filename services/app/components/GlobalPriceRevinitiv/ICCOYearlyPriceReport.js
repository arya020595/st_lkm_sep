...
const REPORT = gql`
  mutation generateYearlyReportForGlobalICCOPrices(
    $fromYearIds: [String!]!
    $toYearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateYearlyReportForGlobalICCOPrices] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateYearlyReportForGlobalICCOPrices({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateYearlyReportForGlobalICCOPrices +
              "?t=" +
