...
const REPORT = gql`
  mutation generateMonthlyReportForGlobalICCOPrices($yearIds: [String!]!) {
    generateMonthlyReportForGlobalICCOPrices(yearIds: $yearIds)
  }
`;
...
const Report = () => {
  ...
  const [generateMonthlyReportForGlobalICCOPrices] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateMonthlyReportForGlobalICCOPrices({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateMonthlyReportForGlobalICCOPrices +
              "?t=" +
