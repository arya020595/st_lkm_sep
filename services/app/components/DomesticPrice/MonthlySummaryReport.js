...
const REPORT = gql`
  mutation generateMonthlySummaryReportForDomesticCocoaPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateMonthlySummaryReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateMonthlySummaryReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
            const reportUrl =
              result.data.generateMonthlySummaryReportForDomesticCocoaPrices +
              "?t=" +
        ...
        <button
          type="submit"
          disabled={!filters.yearIds?.length || !filters.monthIds?.length}
          className={`bg-mantis-600 w-full py-2 rounded-md shadow-md mt-10 ${
            !filters.yearIds?.length || !filters.monthIds?.length
              ? "opacity-50"
              : ""
          }`}>
