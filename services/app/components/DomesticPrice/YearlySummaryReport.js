...
const REPORT = gql`
  mutation generateYearlySummaryReportForDomesticCocoaPrices(
    $yearIds: [String!]!
  ) {
    generateYearlySummaryReportForDomesticCocoaPrices(yearIds: $yearIds)
  }
`;
...
const Report = () => {
  ...
  const [generateYearlySummaryReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateYearlySummaryReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
            const reportUrl =
              result.data.generateYearlySummaryReportForDomesticCocoaPrices +
              "?t=" +
