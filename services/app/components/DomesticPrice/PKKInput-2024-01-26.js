...
const GET_TEMPORARY_PRICE = gql`
  query getDomesticPriceByCentreTemporary($date: String, $centreId: String) {
    getDomesticPriceByCentreTemporary(date: $date, centreId: $centreId) {
      _id
      date
      Centre {
        _id
...
const CREATE_TEMPORARY_DOMESTIC_PRICE = gql`
  mutation createDomesticCocoaPriceTemporary(
    $date: String!
    $buyerId: String!
    $centreId: String!
    $wetPrice: Float
...
const UPDATE_TEMPORARY_DOMESTIC_PRICE = gql`
  mutation updateDomesticCocoaPriceTemporary(
    $_id: String!
    $date: String!
    $buyerId: String!
    $centreId: String!
...
const DELETE_TEMPORARY = gql`
  mutation deleteDomesticCocoaPriceTemporary($_id: String!) {
    deleteDomesticCocoaPriceTemporary(_id: $_id)
  }
`;
...
const PKKInput = ({ currentUserDontHavePrivilege }) => {
  ...
  const [createDomesticCocoaPriceTemporary] = useMutation(
    CREATE_TEMPORARY_DOMESTIC_PRICE,
  );
  const [updateDomesticCocoaPriceTemporary] = useMutation(
    UPDATE_TEMPORARY_DOMESTIC_PRICE,
  );
  const [deleteDomesticCocoaPriceTemporary] = useMutation(DELETE_TEMPORARY);
  ...
  const openCentre = centre => async e => {
    ...
    try {
      ...
      if (currentHour >= 15) {
        ...
        prices = tmp.data.getDomesticPriceByCentreTemporary;
        prices = prices.map(p => {
          return {
            ...p,
            type: "TEMPORARY",
