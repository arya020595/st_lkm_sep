...
const REPORT = gql`
  mutation generateWeeklyCocoaPriceReportForDomesticCocoaPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
    $gradeIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateWeeklyCocoaPriceReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateWeeklyCocoaPriceReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
            const reportUrl =
              result.data.generateWeeklyCocoaPriceReportForDomesticCocoaPrices +
              "?t=" +
