...
const REPORT = gql`
  mutation generateWeeklyAverageReportForDomesticCocoaPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
    $centreIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateWeeklyAverageReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateWeeklyAverageReportForDomesticCocoaPrices(
              {
                variables: {
                  ...filters,
                },
            ...
            const reportUrl =
              result.data.generateWeeklyAverageReportForDomesticCocoaPrices +
              "?t=" +
