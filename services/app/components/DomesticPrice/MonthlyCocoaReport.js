...
const REPORT = gql`
  mutation generateMonthlyCocoaPriceReportForDomesticCocoaPrices(
    $yearIds: [String!]!
    $gradeIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateMonthlyCocoaPriceReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateMonthlyCocoaPriceReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
