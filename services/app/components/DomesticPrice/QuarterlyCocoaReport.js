...
const REPORT = gql`
  mutation generateQuarterlyCocoaPriceReportForDomesticCocoaPrices(
    $yearIds: [String!]!
    $gradeIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateQuarterlyCocoaPriceReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateQuarterlyCocoaPriceReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
        ...
        <button
          type="submit"
          disabled={!filters.gradeIds?.length || !filters.yearIds?.length}
          className={`bg-mantis-600 w-full py-2 rounded-md shadow-md mt-10 ${
            !filters.gradeIds?.length || !filters.yearIds?.length
              ? "opacity-50"
              : ""
          }`}>
