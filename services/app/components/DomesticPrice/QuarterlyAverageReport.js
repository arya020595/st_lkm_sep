...
const REPORT = gql`
  mutation generateQuarterlyAverageReportForDomesticCocoaPrices(
    $yearIds: [String!]!
    $centreIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateQuarterlyAverageReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateQuarterlyAverageReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
            const reportUrl =
              result.data.generateQuarterlyAverageReportForDomesticCocoaPrices +
              "?t=" +
