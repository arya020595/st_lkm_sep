...
const REPORT = gql`
  mutation generateMonthlyBuyerReportForDomesticCocoaPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
    $centreIds: [String!]!
    $buyerIds: [String!]!
...
const Report = () => {
  ...
  const [generateMonthlyBuyerReportForDomesticCocoaPrices] = useMutation(REPORT);
  // console.log({ filters });
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateMonthlyBuyerReportForDomesticCocoaPrices({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateMonthlyBuyerReportForDomesticCocoaPrices +
              "?t=" +
