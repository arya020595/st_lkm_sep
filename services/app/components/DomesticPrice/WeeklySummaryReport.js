...
const REPORT = gql`
  mutation generateWeeklySummaryReportForDomesticCocoaPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
    $weekIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateWeeklySummaryReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateWeeklySummaryReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
            const reportUrl =
              result.data.generateWeeklySummaryReportForDomesticCocoaPrices +
              "?t=" +
            ...
            <Select
              ...
              onChange={selectedValues => {
                ...
                setFilters({
                  ...
                  weekIds: [selectedValues.value],
                  // weeks: [...selectedValues],
                  // weekIds: selectedValues.map(item => item.value),
                });
              }}
            />
          </div>
