...
const REPORT = gql`
  mutation generateDailyAverageReportForDomesticCocoaPrices(
    $monthIds: [String!]!
    $yearIds: [String!]!
    $centreIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateDailyAverageReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateDailyAverageReportForDomesticCocoaPrices(
              {
                variables: {
                  ...filters,
                },
            ...
            const reportUrl =
              result.data.generateDailyAverageReportForDomesticCocoaPrices +
              "?t=" +
