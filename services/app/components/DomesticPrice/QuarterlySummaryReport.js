...
const REPORT = gql`
  mutation generateQuarterlySummaryReportForDomesticCocoaPrices(
    $yearIds: [String!]!
    $quarterIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateQuarterlySummaryReportForDomesticCocoaPrices] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateQuarterlySummaryReportForDomesticCocoaPrices({
                variables: {
                  ...filters,
                },
              });
            const reportUrl =
              result.data.generateQuarterlySummaryReportForDomesticCocoaPrices +
              "?t=" +
            ...
            <Select
              ...
              onChange={selectedValues => {
                ...
                setFilters({
                  ...
                  // quarters: [...selectedValues],
                  // quarterIds: selectedValues.map(item => item.value),
                });
              }}
            />
          </div>
        ...
        <button
          type="submit"
          disabled={!filters.quarterIds?.length || !filters.yearIds?.length}
          className={`bg-mantis-600 w-full py-2 rounded-md shadow-md mt-10 ${
            !filters.quarterIds?.length || !filters.yearIds?.length
              ? "opacity-50"
              : ""
          }`}>
