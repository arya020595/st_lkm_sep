...
const QUERY_GLOBAL_PRODUCTION = gql`
query globalProductionQuery($year: String!, $years: [String!]){
  allGlobalCocoaProductionICCOsTokenized(year: $year, years: $years)
  countGlobalCocoaProductionICCOs
}
`

const CREATE_BCS_GLOBAL_PRODUCTION = gql`
  mutation createGlobalCocoaProductionICCOTokenized(
    $tokenized: String!
  ) {
    createGlobalCocoaProductionICCOTokenized(
      tokenized: $tokenized 
    )
  }
`;
...
const UPDATE_BCS_GLOBAL_PRODUCTION = gql`
  mutation updateGlobalCocoaProductionICCOTokenized(
    $tokenized: String!
  ) {
    updateGlobalCocoaProductionICCOTokenized(
      tokenized: $tokenized
    )
  }
`;
const DELETE_BCS_GLOBAL_PRODUCTION = gql`
  mutation deleteGlobalCocoaProductionICCOTokenized($tokenized: String!) {
    deleteGlobalCocoaProductionICCOTokenized(tokenized: $tokenized)
  }
`;
const BasicCocoaStatisticGlobalProduction = ({
  ...
  let [allGlobalCocoaProductionICCOs, setAllGlobalCocoaProductionICCOs] = useState([])
  let [allCountryRegion, setAllCountryRegion] = useState([])
  let [countGlobalCocoaProductionICCOs, setCountGlobalCocoaProductionICCOs] = useState(0)

  const fetchDataQueyOnce = async () => {
  ...
  const fetchDataGlobalProduction = async (year, years) => {
    const result = await client.query({
      query: QUERY_GLOBAL_PRODUCTION,
      variables: {
        year,
        years
      },
    ...
    const encryptProduction = result.data?.allGlobalCocoaProductionICCOsTokenized
    if (encryptProduction) {
      let Production = []
      const decrypted = jwt.verify(encryptProduction, TOKENIZE);
      Production = decrypted.results
      setAllGlobalCocoaProductionICCOs(Production)
    }
    setCountGlobalCocoaProductionICCOs(result.data?.countGlobalCocoaProductionICCOs)
  }
  useEffect(() => {
    fetchDataGlobalProduction(year, years)
    ...
    <div>
      ...
      <div className="flex mt-4">
        ...
        <p className="text-md font-bold mx-4">
          {countGlobalCocoaProductionICCOs || 0}
        </p>
      </div>
    </div>
  );
