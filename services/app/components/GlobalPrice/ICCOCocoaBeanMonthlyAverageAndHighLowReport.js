...
const REPORT = gql`
  mutation generateCocoaBeanMonthlyAverageAndHighLowReport(
    $yearIds: [String!]!
  ) {
    generateCocoaBeanMonthlyAverageAndHighLowReport(yearIds: $yearIds)
  }
`;
...
const Report = () => {
  ...
  const [generateCocoaBeanMonthlyAverageAndHighLowReport] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateCocoaBeanMonthlyAverageAndHighLowReport({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateCocoaBeanMonthlyAverageAndHighLowReport +
              "?t=" +
        ...
        <button
          type="submit"
          disabled={!filters.yearIds?.length}
          className={`bg-mantis-600 w-full py-2 rounded-md shadow-md mt-10 ${
            !filters.yearIds?.length ? "opacity-50" : ""
          }`}>
