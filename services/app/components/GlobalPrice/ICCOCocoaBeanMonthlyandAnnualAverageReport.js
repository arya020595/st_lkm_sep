...
const REPORT = gql`
  mutation generateCocoaBeanMonthlyandAnnualAverageReport(
    $fromYearIds: [String!]!
    $toYearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateCocoaBeanMonthlyandAnnualAverageReport] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateCocoaBeanMonthlyandAnnualAverageReport({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateCocoaBeanMonthlyandAnnualAverageReport +
              "?t=" +
        ...
        <button
          type="submit"
          disabled={!filters.fromYearIds?.length || !filters.toYearIds?.length}
          className={`bg-mantis-600 w-full py-2 rounded-md shadow-md mt-10 ${
            !filters.fromYearIds?.length || !filters.toYearIds?.length
              ? "opacity-50"
              : ""
          }`}>
