...
const REPORT = gql`
  mutation generateMonthlyInternationalCocoaPriceReport($yearIds: [String!]!) {
...
const Report = () => {
  ...
  const [generateMonthlyInternationalCocoaPriceReport] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateMonthlyInternationalCocoaPriceReport({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateMonthlyInternationalCocoaPriceReport +
              "?t=" +
