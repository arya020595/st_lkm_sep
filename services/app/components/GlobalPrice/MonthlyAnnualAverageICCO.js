...
const QUERY_MONTHLY_ANNUAL_ICCO = gql`
query allMonthlyQuery($year: String, $years: [String!]){
  allMonthlyAnnualAverageICCOTokenized(year: $year, years: $years)
}
`
const CREATE_ANNUAL_AVG_ICCO = gql`
  mutation createMonthlyAverageAnnualICCOPriceTokenized(
    $tokenized: String!
  ) {
    createMonthlyAverageAnnualICCOPriceTokenized(
      tokenized: $tokenized
    )
  }
`;
...
const UPDATE_ANNUAL_AVG_ICCO = gql`
  mutation updateMonthlyAverageAnnualICCOPriceTokenized(
    $tokenized: String!
  ) {
    updateMonthlyAverageAnnualICCOPriceTokenized(
      tokenized: $tokenized
    )
  }
`;
const DELETE_ANNUAL_AVG_ICCO = gql`
  mutation deleteMonthlyAverageAnnualICCOPriceTokenized($tokenized: String!) {
    deleteMonthlyAverageAnnualICCOPriceTokenized(tokenized: $tokenized)
  }
`;
const MonthlyAnnualAverageICCO = ({ currentUserDontHavePrivilege }) => {
  ...
  let [allMonthlyAnnualAverageICCO, setAllMonthlyAnnualAverageICCO] = useState([])
  ...
  const fetchDataMonthlyAnnual = async (year, years) => {
    const result = await client.query({
      query: QUERY_MONTHLY_ANNUAL_ICCO,
      variables: {
        year,
        years
      },
    ...
    const encryptMonthlyAnnualAverage = result.data?.allMonthlyAnnualAverageICCOTokenized
    if (encryptMonthlyAnnualAverage) {
      let MonthlyAnnualAverage = []
      const decrypted = jwt.verify(encryptMonthlyAnnualAverage, TOKENIZE);
      MonthlyAnnualAverage = decrypted.results
      setAllMonthlyAnnualAverageICCO(MonthlyAnnualAverage)
    }
  }

  useEffect(() => {
    fetchDataMonthlyAnnual()
        ...
        onSubmit={async e => {
          ...
          try {
            ...
            if (!_id) {
              ...
              let tokenized = jwt.sign(payload, TOKENIZE); await createMonthlyAverageAnnualICCOPrice({
                variables: {
                  tokenized
                },
              });
