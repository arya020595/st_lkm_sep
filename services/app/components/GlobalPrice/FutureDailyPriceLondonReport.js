...
const REPORT = gql`
  mutation generateDailyInternationalCocoaPriceReport(
    $monthIds: [String!]!
    $yearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateDailyInternationalCocoaPriceReport] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateDailyInternationalCocoaPriceReport({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateDailyInternationalCocoaPriceReport +
              "?t=" +
