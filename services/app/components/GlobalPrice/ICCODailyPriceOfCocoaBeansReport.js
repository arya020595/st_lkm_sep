...
const REPORT = gql`
  mutation generateICCODailyPriceOfCocoaBeansReport(
    $yearIds: [String!]!
    $currencies: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateICCODailyPriceOfCocoaBeansReport] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateICCODailyPriceOfCocoaBeansReport({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateICCODailyPriceOfCocoaBeansReport +
              "?t=" +
          ...
          <div className="form-group">
            ...
            <Select
              ...
              onChange={selectedValues => {
                ...
                setFilters({
                  ...filters,
                  currencies: [selectedValues.value],
                });
              }}
            />
          </div>
        ...
        <button
          type="submit"
          disabled={!filters.yearIds?.length || !filters.currencies?.length}
          className={`bg-mantis-600 w-full py-2 rounded-md shadow-md mt-10 ${
            !filters.yearIds?.length || !filters.currencies?.length
              ? "opacity-50"
              : ""
          }`}>
