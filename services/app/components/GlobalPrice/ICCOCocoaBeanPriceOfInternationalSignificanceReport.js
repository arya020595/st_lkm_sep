...
const REPORT = gql`
  mutation generateCocoaBeanPriceOfInternationalSignificanceReport(
    $fromYearIds: [String!]!
    $toYearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateCocoaBeanPriceOfInternationalSignificanceReport] =
    useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          try {
            let result =
              await generateCocoaBeanPriceOfInternationalSignificanceReport({
                variables: {
                  ...filters,
                },
              });
