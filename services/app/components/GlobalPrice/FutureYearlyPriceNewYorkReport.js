...
const REPORT = gql`
  mutation generateYearlyInternationalCocoaPriceReport(
    $fromYearIds: [String!]!
    $toYearIds: [String!]!
  ) {
...
const Report = () => {
  ...
  const [generateYearlyInternationalCocoaPriceReport] = useMutation(REPORT);
    ...
    <div className="w-full py-4 flex flex-col md:flex-row">
      <form
        onSubmit={async e => {
          ...
          showLoadingSpinner();
          try {
            let result = await generateYearlyInternationalCocoaPriceReport({
              variables: {
                ...filters,
              },
            });
            const reportUrl =
              result.data.generateYearlyInternationalCocoaPriceReport +
              "?t=" +
