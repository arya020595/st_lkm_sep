...
const QUERY = gql`
  query listQueries($year: String!, $years: [String!]) {
    allEmployments(year: $year, years: $years) {
      _id
      year
      Category {
        _id
...
const DELETE_BCS_EMPLOYMENT = gql`
  mutation deleteEmployment($_id: String!) {
...
const BasicCocoaStatisticCultivatedArea = ({
  ...
  const [createEmployment] = useMutation(CREATE_BCS_EMPLOYMENT);
  const [updateEmployment] = useMutation(UPDATE_BCS_EMPLOYMENT);
  const [deleteEmployment] = useMutation(DELETE_BCS_EMPLOYMENT);
  let allEmployments = [];
  ...
  return (
    <div>
      ...
      <div className="flex mt-4">
        ...
        <p className="text-md font-bold mx-4">{data?.countEmployments || 0}</p>
      </div>
    </div>
  );
};
