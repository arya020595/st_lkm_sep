...
const QUERY = gql`
  query listQueries($year: String!, $years: [String!]) {
    allBasicCocoaStatisticDomesticProductionsTokenized(
      year: $year
      years: $years
    )
...
const CREATE_BCS_DOMESTIC_PRODUCTION = gql`
  mutation createBasicCocoaStatisticDomesticProductionTokenized(
    $tokenizedInput: String!
  ) {
    createBasicCocoaStatisticDomesticProductionTokenized(
...
const UPDATE_BCS_DOMESTIC_PRODUCTION = gql`
  mutation updateBasicCocoaStatisticDomesticProductionTokenized(
    $tokenizedInput: String!
  ) {
    updateBasicCocoaStatisticDomesticProductionTokenized(
      tokenizedInput: $tokenizedInput
    )
  }
`;
const DELETE_BCS_DOMESTIC_PRODUCTION = gql`
  mutation deleteBasicCocoaStatisticDomesticProductionTokenized(
    $tokenizedInput: String!
  ) {
    deleteBasicCocoaStatisticDomesticProductionTokenized(
      tokenizedInput: $tokenizedInput
    )
  }
`;
...
const BasicCocoaStatisticDomesticProduction = ({
  ...
  const [allBasicCocoaStatisticDomesticProductions, setBCSDomesticProduction] =
    useState([]);
  const [countBasicCocoaStatisticDomesticProductions, setCountData] =
    useState(0);
  ...
  // console.log({ years });
  const [createBasicCocoaStatisticDomesticProductionTokenized] = useMutation(
    CREATE_BCS_DOMESTIC_PRODUCTION,
  );
  const [updateBasicCocoaStatisticDomesticProductionTokenized] = useMutation(
    UPDATE_BCS_DOMESTIC_PRODUCTION,
  );
  const [deleteBasicCocoaStatisticDomesticProductionTokenized] = useMutation(
  ...
  const fetchDataQuery = async (year, years) => {
    ...
    const encryptedData =
      result.data?.allBasicCocoaStatisticDomesticProductionsTokenized || "";
    if (encryptedData) {
      const decrypted = jwt.verify(encryptedData, TOKENIZE);
      setBCSDomesticProduction(decrypted.results);
    }
    setCountData(result.data.countBasicCocoaStatisticDomesticProductions);
  }
  useEffect(() => {
    showLoadingSpinner();
    try {
    ...
    <div>
        ...
        onSubmit={async e => {
          ...
          try {
            ...
            if (!_id) {
              ...
              await createBasicCocoaStatisticDomesticProductionTokenized({
                variables: {
                  tokenizedInput,
                },
              });
              ...
              await updateBasicCocoaStatisticDomesticProductionTokenized({
                variables: {
                  tokenizedInput,
                },
              });
              ...
              try {
                ...
                if (yes) {
                  for (const row of rows) {
                    ...
                    await deleteBasicCocoaStatisticDomesticProductionTokenized(
                      {
                        variables: {
                          tokenizedInput,
                        },
      ...
      <div className="flex mt-4">
        ...
        <p className="text-md font-bold mx-4">
          {countBasicCocoaStatisticDomesticProductions || 0}
        </p>
      </div>
    </div>
  );
